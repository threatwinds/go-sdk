...
func TestValidateSHA512224(t *testing.T) {
	validCases := []struct {
		name     string
		input    string
		expected string
	...
	for _, tc := range validCases {
		t.Run(tc.name, func(t *testing.T) {
			actual, _, err := ValidateSHA512224(tc.input)
			if err != nil {
	...
	for _, tc := range invalidCases {
		t.Run(tc.name, func(t *testing.T) {
			_, _, err := ValidateSHA512224(tc.input)
			if err == nil {
